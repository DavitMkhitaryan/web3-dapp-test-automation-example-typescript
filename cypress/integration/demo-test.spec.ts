...
describe("Securrency DApp Demo Tests", () => {
  before(() => {
    cy.setupMetamask(
      metamaskSetupData.secretWords,
      metamaskSetupData.network,
      metamaskSetupData.password
    );
    ...
    cy.getMetamaskWalletAddress().then(($address) => {
      homePage
        ...
        .should("have.value", $address.toLowerCase());
    });
  });
  ...
  it("Verify Header: Connected State", () => {
    ...
    homePage
      .getAddCitizenButton()
      .should("have.text", expectedTexts.addCitizenButtonText);
  ...
  it('Verify "Citizens List" Table Default Layout', () => {
    ...
    homePage
      .getCitizensListText()
      .should("have.text", expectedTexts.citizensListText);

    // Verify "total records : {number}" table header text is displayed
    homePage.getTableResultsText().should("be.visible");
    homePage
      .getTableResultsText()
      .should("contain.text", expectedTexts.totalRecordsText);

    // Verify Table Column Titles are correctly displayed
    homePage
      .getTableHeaderColumnValue1()
      .should("have.text", expectedTexts.tableHeaderColumnText1);
    homePage
      .getTableHeaderColumnValue2()
      .should("have.text", expectedTexts.tableHeaderColumnText2);
    homePage
      .getTableHeaderColumnValue3()
      .should("have.text", expectedTexts.tableHeaderColumnText3);
    homePage
      .getTableHeaderColumnValue4()
      .should("have.text", expectedTexts.tableHeaderColumnText4);
